...
public:
    ...
    // Aporte: BMSSP-LightPQ
    // Para lotes pequeños se evita convertir vector -> list.
    // Se inserta directamente en D0, igual que en batchPrepend(list).
    if(sz <= M){
    ...
    // Para lotes grandes se mantiene el comportamiento original.
    std::list<elementT> l;
    for(const auto &x: v){
...
private:
    ...
    // Aporte: BMSSP-DedupPivots
    // Evita insertar varias veces el mismo vertice en nw_active.
    int counter_active = 0;
    std::vector<int> active_vis;
    std::pair<std::vector<int>, std::vector<int>> findPivots(uniqueDistT B, const std::vector<int> &S) { // Algorithm 1
        ...
        for(int i = 1; i <= k; i++) {
                ...
                for(auto [v, w]: adj[u]) {
                    if(getDist(u, v, w) <= getDist(v)) {
                        ...
                        if(getDist(v) < B) {
                            ...
                            // Aporte: evitar vertices duplicados en la siguiente frontera activa.
                            if(active_vis[v] != counter_active) {
                                active_vis[v] = counter_active;
            ...
            // Aporte: parada temprana si ya no hay vertices activos.
    ...
    std::pair<uniqueDistT, std::vector<int>> cutoffDijkstra(uniqueDistT B, const std::vector<int> &S) {
        std::vector<int> complete;
        std::priority_queue<uniqueDistT, std::vector<uniqueDistT>, std::greater<uniqueDistT>> heap;
    ...
    std::pair<uniqueDistT, std::vector<int>> bmsspRec(short int l, uniqueDistT B, const std::vector<int> &S) { // Algorithm 3
        ...
        if(use_cutoff && ((int)S.size() <= cutoff_threshold)) {
            return cutoffDijkstra(B, S);
        }
